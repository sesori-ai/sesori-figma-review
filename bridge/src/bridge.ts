...
function ask(args: {
  ...
  return new Promise(resolve => {
    if (!activeOwners.has(args.owner)) return resolve(args.onDrop);
...
function cancelRequests(args: { owner: string; reason: string }) {
  for (const [id, request] of pending) {
    ...
    if (live(request.ws)) request.ws.send(JSON.stringify({ kind: "cancel_request", id, reason: args.reason } satisfies DownMsg));
  }
}
function deactivate(args: { owner: string; reason: string }) {
  activeOwners.delete(args.owner);
  cancelRequests(args);
}
...
const dormantBoundary: ProviderRequestBoundary = {
  tool: async () => disconnected,
...
function prepareProvider(args: { provider: ReviewProvider; fileId: string; dir: string }) {
  const settings = readSettings().providers[args.provider.id];
  // Warm Claude cannot call tools. Its boundary delegate is replaced atomically by start().
  args.provider.prepare({ fileId: args.fileId, dir: args.dir, settings, boundary: dormantBoundary });
}
  ...
  try {
    const activated = await activateProvider({
      ...
      accept: session => {
        ...
        void pump(current);
        prepareProvider({ provider, fileId: message.fileId, dir });
      },
    });
...
async function onUp(ws: WebSocket & { fileId?: string; protocolOk?: boolean }, message: UpMsg) {
  ...
  switch (message.kind) {
    case "hello": {
      ...
      const settings = readSettings(), selected = providers[settings.provider];
      if (selected) prepareProvider({ provider: selected, fileId: message.fileId, dir });
    ...
    case "settings": {
      ...
      const target = conv;
      const activeChanged = !!target && JSON.stringify(previous.providers[target.record.provider])
        !== JSON.stringify(message.settings.providers[target.record.provider]);
      if (target && activeChanged) {
        try { await target.session.applySettings({ settings: message.settings.providers[target.record.provider] }); }
        catch (error) {
          if (conv === target) {
            send(target.fileId, { kind: "error", message: `Settings update failed: ${error instanceof Error ? error.message : String(error)}` });
            send(target.fileId, { kind: "health", health: makeHealth() });
          }
          return;
        }
      ...
      for (const providerId of ["claude", "codex"] as const) {
        if (JSON.stringify(previous.providers[providerId]) !== JSON.stringify(message.settings.providers[providerId])) providers[providerId]?.dispose();
      }
      if (ws.fileId) {
        const selected = providers[message.settings.provider];
        if (selected) prepareProvider({ provider: selected, fileId: ws.fileId, dir: workspaceFor(ws.fileId, "Figma file") });
        send(ws.fileId, { kind: "health", health: makeHealth() });
      }
      return;
    }
...
server.on("listening", () => log(`bridge ${VERSION} listening on ws://127.0.0.1:${port}` + (process.env.APP_REPO ? ` · app repo ${process.env.APP_REPO}` : "")));
server.on("connection", (ws: WebSocket & { fileId?: string; protocolOk?: boolean }) => {
