...
import { hasClaudeAuth, installPlugin, readSettings } from "./workspace.ts";
...
if (readSettings().provider === "claude" && !hasClaudeAuth()) console.log("No Claude credentials found: run `claude` once to sign in, or export ANTHROPIC_API_KEY. The plugin will show \"Claude failed to start\" until then.\n");
