...
const assertVersion = (userAgent: string) => {
  const version = versionFromUserAgent(userAgent);
...
/** Read transient names for one frozen-config review lifetime. No MCP status, thread, or model call occurs. */
export async function discoverCodexRuntime(args: {
  client: CodexClient;
  policy: CodexExecutionPolicy;
}): Promise<CodexIsolationInventory> {
  assertVersion((await args.client.connect()).userAgent);
  ...
  return discoverCodexIsolation(effective);
}
...
export async function qualifyCodexRuntime(args: {
  ...
  const version = assertVersion((await args.client.connect()).userAgent);
