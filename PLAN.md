...
| Per-file workspace under `~/.local/share/sesori-figma-review/files/<fileId>/` as the agent's cwd | Gives CLAUDE.md, a skill, `.mcp.json`, notes/ and a sessions index per Figma file; keeps writes away from the app repo (which is mounted read-only as an additional directory). |
