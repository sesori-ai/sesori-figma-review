...
export class ConversationView {
  ...
  get awaiting(): boolean { return !!this.intent; }
  ...
  reconcile(args: { intentId?: string; session?: SessionRecord }): { queued: QueuedInput[]; cancelled: QueuedInput[] } {
    ...
    if (this.intent?.kind === "start") {
      ...
      this.intent = undefined; // native read was interrupted; a late response must not replace a newer view
    }
    if (args.session?.sessionId) this.current = args.session;
    else if (args.intentId && args.session) {
      this.current = undefined;
      this.intent = { kind: "start", id: args.intentId, inputs: [] }; // fresh plugin attached during bridge startup
    }
